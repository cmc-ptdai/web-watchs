...
import { setEvaluate as setEvaluateAction } from '../../../redux/actions/products';
...
const Evaluate = ({ data }) => {
  ...
  const totalPoint = () => {
    ...
    return totalPoint / evaluateDefault.evaluates.length;
  };
  ...
  return (
    <>
      {evaluateDefault !== null && (
        <>
          <p className="evaluate-title">Đánh giá sản phẩm</p>
          <div className="evaluate-content">
            <div className="evaluate-content-rate">
              <div>
                <p style={{ margin: '0' }}>
                  Tổng số đánh giá {evaluateDefault ? evaluateDefault.evaluates.length : 0} (đánh
                  giá)
                </p>
                <span style={{ marginRight: '5px' }}>{totalPoint()} trên 5 :</span>
                <Rate disabled value={totalPoint()} />
              </div>
              <div>
                <Rate disabled tooltips={desc} value={5} />
                <span className="ant-rate-text">
                  {desc[4]}&nbsp;( {totalCount(5)} )
                </span>
              </div>
              <div>
                <Rate disabled tooltips={desc} value={4} />
                <span className="ant-rate-text">
                  {desc[3]}&nbsp;( {totalCount(4)} )
                </span>
              </div>
              <div>
                <Rate disabled tooltips={desc} value={3} />
                <span className="ant-rate-text">
                  {desc[2]} &nbsp;( {totalCount(3)} )
                </span>
              </div>
              <div>
                <Rate disabled tooltips={desc} value={2} />
                <span className="ant-rate-text">
                  {desc[1]} &nbsp;( {totalCount(2)} )
                </span>
              </div>
              <div>
                <Rate disabled tooltips={desc} value={1} />
                <span className="ant-rate-text">
                  {desc[0]} &nbsp;( {totalCount(1)} )
                </span>
              </div>
              <div className="evaluate-content-rate-groupButton">
                <span className={user && (user.id === undefined ? 'evaluateDisable' : 'evaluate')}>
                  <button
                    onClick={showModal}
                    disabled={user && (user.id === undefined ? true : false)}
                  >
                    Đánh giá sản phẩm
                  </button>
                  <span style={{ display: user && (user.id === undefined ? 'block' : 'none') }}>
                    ( Đăng nhập để gửi đánh giá của bạn <Link to="/login">Đăng nhập tại đây</Link>)
                  </span>
                </span>
              </div>
            </div>
          ...
          <div className="fromEvaluate">
            ...
            >
              ...
              {fetchPointDefault(user.id) && !showEdit ? (
                <>
                  <Rate disabled value={fetchPointDefault(user.id).point} />
                ...
                <>
                  <Rate onChange={handleChange} value={evaluate} />
                  <Input
