...
const openNotification = () => {
  notification.open({
    message: '',
    description: (
      <span style={{ display: 'block', height: '60px', fontSize: '15px' }}>
...
const SingUp = () => {
  ...
  return (
    <div className="singUp">
      <Form form={form} name="singUp" {...layout} onFinish={onFinish}>
        <Form.Item
          label="Họ Tên"
          name="name"
          rules={[
        ...
        >
          <Select placeholder="Select a option and change gender" allowClear>
            ...
            ({ getFieldValue }) => ({
              validator(rule, value = '') {
                const user = listUser.filter(
                  (item) => item.userName.toLowerCase() === value.toLowerCase()
                );
                if (user.length > 0) {
        ...
        <Form.Item {...tailLayout}>
          <Link to="/login">
            <Button type="primary">Quay lại trang đăng nhập</Button>
          </Link>
